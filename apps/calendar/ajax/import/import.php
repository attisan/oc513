...
if($_POST['method'] == 'new') {
	...
	if($newcal) {
		$id = OCA\Calendar\Calendar::addCalendar(OCP\USER::getUser(), strip_tags($_POST['calname']),'VEVENT,VTODO,VJOURNAL',null,0,strip_tags($_POST['calcolor']));
		OCA\Calendar\Calendar::setCalendarActive($id, 1);
	}
}else{
	$calendar = OCA\Calendar\App::getCalendar($_POST['id']);
...
try{
	...
	OCP\JSON::error(array('message'=>OCA\Calendar\App::$l10n->t('Import failed'), 'debug'=>$e->getMessage()));
	//write some log
}
$count = $import->getCount();
if($count == 0) {
	if($newcal) {
		OCA\Calendar\Calendar::deleteCalendar($id);
	}
	...
	if($newcal) {
		OCP\JSON::success(array('message'=>$count . ' ' . OCA\Calendar\App::$l10n->t('events has been saved in the new calendar') . ' ' .  strip_tags($_POST['calname'])));
	}else{
		OCP\JSON::success(array('message'=>$count . ' ' . OCA\Calendar\App::$l10n->t('events has been saved in your calendar')));
	}
}
