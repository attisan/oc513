		...
		//foreach($calendars as $calendar) {
			...
			foreach($objects as $object) {
				...
				if(array_key_exists($query,$allowedCommands) && $allowedCommands[$query]){
					if($query=='#dt'){
						...
						if(substr_count($search,$today)>0){
							$searchAdvanced=true;
							\OCP\Util::writeLog('calendar','search: ->'.$search.':'.$query, \OCP\Util::DEBUG);
						}
					}
				...
				if(substr_count(strtolower($object['summary']), strtolower($query)) > 0 || $searchAdvanced==true) {
					...
					$link = \OCP\Util::linkTo('aufgaben', 'index.php').'#'.urlencode($object['id']);
					$results[]=new \OC_Search_Result($object['summary'],$info, $link,(string)$l->t('Task'));//$name,$text,$link,$type
				}
			}
		//}
		return $results;
